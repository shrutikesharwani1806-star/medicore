...
import { Coins, QrCode, ArrowLeft, CheckCircle, Clock, CreditCard, Zap } from 'lucide-react';
...
export default function BuyCreditsPage() {
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      ...
      const options = {
        key: import.meta.env.VITE_RAZORPAY_KEY_ID || 'rzp_test_dummy_key_id', // Add this in client env
      ...
      <div className="flex items-center justify-center min-h-[60vh] animate-scale-in">
        <div className="text-center max-w-sm">
          ...
          <p className="text-xs text-green-600 bg-green-50 rounded-xl px-3 py-2 mb-4">
            ✅ Credits have been added to your account instantly.
          </p>
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="animate-slide-up">
        ...
        <p className="text-sm text-slate-500">Add credits to your account to book appointments</p>
      </div>

      {/* Current Balance */}
      <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-5 text-white animate-slide-up">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
            <Coins className="w-6 h-6" />
          </div>
          <div>
            <p className="text-amber-100 text-xs">Current Balance</p>
            <p className="text-3xl font-bold">₹{user?.credits || 0}</p>
          </div>
        </div>
        <p className="text-amber-100 text-xs mt-3">
          💡 Booking an appointment costs ₹200 (₹150 consultation + ₹50 booking fee). Note: A 10% processing fee applies on all credit purchases.
        </p>
      </div>
      ...
      <div className="animate-slide-up" style={{ animationDelay: '200ms' }}>
        ...
        >
          <Zap className="w-5 h-5" /> Pay ₹{amount} instantly with Razorpay
        </Button>
        <p className="text-xs text-center text-slate-500">
          You will receive <span className="font-bold text-slate-700">₹{amount - Math.floor(amount * 0.1)}</span> credits (after 10% fee).
        </p>
      </div>
            ...
            {paymentHistory.slice(0, 20).map((p) => (
              <div key={p._id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-700">{p.description || p.type}</p>
